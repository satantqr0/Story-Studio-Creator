...
Story Studio Creator 适配本地模型和可配置的云端/本地语言模型 API。

- 项目创建时可选择画面比例与贯穿流程的生产分辨率；支持横屏、竖屏和方形工作流。
- 图生视频可按原图比例选择自动分辨率，不强制裁切为固定画幅。
- 生成任务在统一队列中可见；GPU 密集任务按资源策略顺序执行。
- 可查看任务状态、排队进度、存储占用、资产来源与可安全删除范围。
- 模型运行前可执行预检；任务完成、失败或主动释放时可回收项目占用的运行时与显存。
- 图像、视频、补帧和导出环节可由本地 ComfyUI / FFmpeg 等运行时协同完成；实际效果取决于安装模型、显存、分辨率、提示词和参考素材质量。
